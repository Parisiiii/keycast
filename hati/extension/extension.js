...
export default class HatiExtension extends Extension {
  ...
  _tick() {
    ...
    // auto-hide disabled: visibility is driven by click state below
    ...
    // only show highlight while a mouse button is pressed (fades via clickState)
    const visibility = this._clickState.active ? this._clickState.progress : 0;
    this._containerActor.set_opacity(Math.round(255 * visibility));
